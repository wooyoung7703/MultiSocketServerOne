...
/* JavaScript content from js/LCMWC/LCMWC200.js in folder common */
...
pageObj.initCertInfo = function(){
	...
	pageObj.certInfo.members_active = "";	// 멤버스 유효카드 존재여부
		
	pageObj.certInfo.members_cdno = "";
	pageObj.certInfo.members_home_post_no = "";
	pageObj.certInfo.members_home_pn_add = "";
	pageObj.certInfo.members_home_bpsno_add = "";
	pageObj.certInfo.members_mail_id = "";
...
pageObj.resultLCMWC210 = function(resultData){
	...
	// 인증업체 약관
	consoleLog("D","resultData : "+resultData);
...
pageObj.certNumSend = function(){
	...
	userInfo.rr_gender  = pageObj.certInfo.rr_gender;
...
pageObj.certNumChk = function(){
	...
	if(jq("#rd_no").val() == "" || jq("#rd_no").val().length != 6){
...
pageObj.successCallphone =function(){
    goMain();
};

pageObj.card_cert_chk = function(){
    var check = false;
    if(userInfo.card_cert==undefined||userInfo.card_cert==""||userInfo.card_cert!="Y")
        check = true;
    return check;
};
